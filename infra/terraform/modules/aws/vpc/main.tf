...
resource "aws_route_table" "private" {
  count  = var.enable_nat_gateway && length(aws_nat_gateway.this) > 0 ? 1 : 0
  vpc_id = aws_vpc.this.id
  route {
    cidr_block     = "0.0.0.0/0"
...
resource "aws_route_table_association" "private" {
  count          = var.enable_nat_gateway && length(aws_route_table.private) > 0 ? length(aws_subnet.private) : 0
